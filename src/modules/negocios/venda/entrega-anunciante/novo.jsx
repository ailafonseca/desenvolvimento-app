...
class EntregaAnuncianteEditar extends Component {
  ...
  componentDidMount () {
    const { match: { params }, dispatch, loadReservaById } = this.props
  ...
  renderObservacoesAnuncio = (anuncio) => {
    if (anuncio.observacaoAnuncio !== '') {
      ...
      return (<>Sem observações de anúncio<br /></>)
    }
  }
  ...
  calculateTotalPrice () {
    ...
    for (let i = 0; i < reservaProdutos.length; i++) {
      precoTotal = precoTotal + reservaProdutos[i].quantidadeRetiradaAnunciante * reservaProdutos[i].precoUnitario
    }
  ...
  renderFailAlert () {
    ...
    const { reservaProdutos } = this.state
    for (let i = 0; i < reservaProdutos.length; i++) {
      if (isFailedToSaveEntrega === true && (reservaProdutos[i].qtdReservada !== reservaProdutos[i].quantidadeRetiradaAnunciante)) { return <Alert variant='danger'>Indique nas observações o motivo da divergência na quantidade do produto reservado e entregue.</Alert> }
    }
  }
        ...
        <div className='row justify-content-end'>
          ...
          <div className='col-md-3 my-2'>
            <BotaoSalvar
              ...
              disabled={pristine || submitting || invalid || isLoadingBtn || isNullOrEmpty(this.state.responsavelEntrega)}
            />
          </div>
        </div>
      )
