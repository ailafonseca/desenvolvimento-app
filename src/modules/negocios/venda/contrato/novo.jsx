...
class VendaContratoNovo extends React.Component {
  constructor(props) {
    super(props)
  ...
  handleChangePreco = (idProduto, preco) => {
    ...
    if (index >= 0) produtos[index].preco = preco
    this.setState({ produtos })
  }
    ...
    if (pagamentos !== undefined && pagamentos.constructor === Array && pagamentos.length > 0) {
      const filteredPagamentos =
        pagamentosJaSelecionados !== undefined && pagamentosJaSelecionados.constructor === Array && pagamentosJaSelecionados.length > 0
          ? pagamentos.filter((pagamento) => !pagamentosJaSelecionados.includes(pagamento))
          : pagamentos

      if (filteredPagamentos !== undefined && filteredPagamentos.constructor === Array && filteredPagamentos.length > 0) {
        const optionsPagamentos = filteredPagamentos.map((item) => {
  ...
  handlePaymentAdd = (event) => {
    ...
    if (Array.isArray(pagamentosList)) {
      const pagamento = pagamentosList.find((p) => p.id === idPagamento)
    ...
    if (formasDePagamento !== undefined && formasDePagamento.constructor === Array && formasDePagamento.length > 0) {
      return (
        <table className="table table-hover table-sm">
          <tbody>
            {formasDePagamento.map((pagamento, i) => {
              return (
                <tr key={i}>
                  <td className="align-middle">{pagamento.nome}</td>
                  <td className="align-middle text-right">
                    <button type="button" className="btn btn-link btn-sm" onClick={() => this.handlePaymentRemove(pagamento.id)}>
  ...
  handlePaymentRemove = (idPagamento) => {
    if (Array.isArray(this.state.formasDePagamento)) {
      const pagamentos = this.state.formasDePagamento.filter((pagamento) => pagamento.id !== idPagamento)
  ...
  renderModalSemProd() {
    ...
    if (this.state.produtos.length <= 0) {
      return (
        <Modal show={this.state.showModalSemProd} onHide={() => this.setState({ showModalSemProd: false })} size="lg" centered aria-labelledby="contained-modal-title-vcenter">
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">Seu contrato está sem produtos.</Modal.Title>
          </Modal.Header>
  ...
  handleEtapa1 = () => {
    ...
    return (
      <RenderEtapa1
        ...
        ativo={checked}
        setAtivo={(value) => this.setState({ checked: value })}
      />
    )
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(VendaContratoNovo)
