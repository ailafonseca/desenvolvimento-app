...
export default class ProdutosCadastradosList extends React.Component {
    ...
    renderProdutoInserido = (produto) => (
      <ProdutoItem
        ...
        isPrecoProduto={this.props.isPrecoProduto}
    ...
    renderAddNew = () => {
      ...
      if (onAddMode) {
        return (
          <ProdutoNovo
            produtos={produtos}
            onCancel={() => this.setState({ onAddMode: false })}
    ...
    renderAddNewButton = () => (
      <div className='text-right'>
        <button className='btn btn-sm btn-primary' onClick={() => this.setState({ onAddMode: true })}>Adicionar novo produto manualmente</button>
      </div>
    )
