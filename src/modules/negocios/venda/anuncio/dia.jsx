...
class VendaRecorrenteAnuncioNovo extends React.Component {
  componentDidMount () {
  ...
  renderNovoAnuncio = (line) => (
    <Link to={`${URL_VENDA.ANUNCIO_EDITAR_BASE}/${line.id}`} className='btn-secondary btn-lg mobile'>
      Novo Anúncio
    </Link>
  );

  renderNovoAnuncioMobile = (line) => (
    <Link to={`${URL_VENDA.ANUNCIO_EDITAR_BASE}/${line.id}`} className='btn-secondary btn-lg botao2 button2'>
...
const mapStateToProps = (state) => ({
  ...
  diaFailure: state.vendaAnuncio.diaFailure,
  listDia: state.vendaAnuncio.listDia
})

const mapDispatchToProps = (dispatch) => ({ dispatch, redirectToContratoDoDia })
export default connect(mapStateToProps, mapDispatchToProps)(VendaRecorrenteAnuncioNovo)
