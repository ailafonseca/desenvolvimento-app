...
import { loadEntregasDonatario } from './action'
...
class DoacaoListaEntregas extends React.Component {
  componentDidMount () {
    const { dispatch } = this.props
    dispatch(loadEntregasDonatario())
  }
  ...
  renderBodyRow = (reserva) => (
    <ListBodyRow key={reserva.anuncio.id}>
      ...
      <ListBodyCol last format='actionButtons'>
        <ActionButton to={`/doacao/entrega-donatario/novo/${reserva.idUltimaInteracao}`} icon={faPen} hidden={reserva.anuncio.reservas.length !== 0} />
      </ListBodyCol>
    </ListBodyRow>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(DoacaoListaEntregas)
